...
class SupervisorAgent:
    ...
    def __init__(self, config_path: str, azure_endpoint: str = None, 
                 azure_deployment: str = "gpt-4", api_version: str = "2024-02-15-preview"):
...
"""
    ...
    def _build_graph(self) -> StateGraph:
        ...
        # Add supervisor direct response node
        workflow.add_node("supervisor_response", self._supervisor_direct_response)
        ...
        # Create routing dictionary that includes all agents plus supervisor option
        routing_dict = {agent_name: agent_name for agent_name in self.agents.keys()}
        routing_dict["SUPERVISOR"] = "supervisor_response"
        ...
        # Route supervisor response to synthesis
        workflow.add_edge("supervisor_response", "synthesize")
    ...
    def _supervisor_direct_response(self, state: SupervisorState) -> SupervisorState:
        """Handle cases where supervisor responds directly"""
        current_message = state["messages"][-1] if state["messages"] else ""
        
        try:
            # Create a direct response prompt
            direct_response_prompt = f"""You are a helpful AI assistant. The user's message couldn't be routed to a specialized agent, so you should provide a direct, helpful response.

User message: {current_message}

Provide a clear, helpful response to the user's query."""

            response = self.llm.invoke(direct_response_prompt)
            state["final_response"] = response.content
            
            logger.info("Supervisor provided direct response")
            
        except Exception as e:
            logger.error(f"Error in supervisor direct response: {e}")
            state["final_response"] = "I'm sorry, I encountered an error while processing your request."
        
        return state
    
    def _route_decision(self, state: SupervisorState) -> str:
        """Determine routing based on supervisor decision"""
        routing_decision = state.get("routing_decision", "SUPERVISOR")
        
        # Ensure the routing decision is valid
        valid_routes = list(self.agents.keys()) + ["SUPERVISOR"]
        if routing_decision not in valid_routes:
            logger.warning(f"Invalid routing decision: {routing_decision}, defaulting to SUPERVISOR")
            routing_decision = "SUPERVISOR"
...
def main():
    ...
    parser.add_argument("--azure-deployment", default="gpt-4", help="Azure OpenAI deployment name")
